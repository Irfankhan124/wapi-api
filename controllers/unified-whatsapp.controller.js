  ...
  try {
    ...
    if (!waMessageId) {
      return res.status(502).json({
        success: false,
        error: 'WhatsApp did not confirm message submission',
        details: 'The provider returned no real WhatsApp message ID.',
        data: result
      });
    }

    const savedMessage = messageId ? await Message.findById(messageId).lean() : null;
    const formattedResponse = savedMessage
      ? {
        ...
        wa_message_id: savedMessage.wa_message_id || waMessageId,
    ...
    const details = String(error?.message || 'Failed to send message');
    const normalized = details.toLowerCase();
    const statusCode =
      normalized.includes('not actually connected') || normalized.includes('reconnect by qr')
        ? 409
        : normalized.includes('not registered') || normalized.includes('could not be verified')
          ? 422
          : normalized.includes('invalid recipient') || normalized.includes('required')
            ? 400
            : 500;
    ...
    return res.status(statusCode).json({
      ...
      error: details,
      details,
      connection_error: statusCode === 409,
      recipient_verified: statusCode === 422 ? false : undefined
    });
  }
};
...
export const getConnectionStatus = async (req, res) => {
  try {
    ...
    } else if (whatsappPhoneNumberId) {
      phoneNumber = await WhatsappPhoneNumber.findOne({
        _id: whatsappPhoneNumberId,
        user_id: userId,
        deleted_at: null
      }).populate('waba_id');
      waba = phoneNumber?.waba_id || null;
    } else {
      waba = await WhatsappWaba.findOne({
        user_id: userId,
        is_active: true,
    ...
    const providerType = waba.provider || provider || PROVIDER_TYPES.BUSINESS_API;
    const runtime = await unifiedWhatsAppService.getConnectionStatus(
      userId,
      providerType,
      waba._id.toString()
    );
    ...
    return res.json({
      ...
      status: runtime.status || waba.connection_status || 'unknown',
      stored_status: waba.connection_status || null,
      runtime_state: runtime.runtime_state || null,
      verified_live_socket: runtime.verified_live_socket === true,
      connection_id: waba._id.toString(),
...
export const connectWhatsApp = async (req, res) => {
  try {
    ...
    if (provider === PROVIDER_TYPES.BUSINESS_API) {
      ...
      const finalInstanceName = String(instance_name || bodyName || '').trim();
      ...
      // One WAPI account should reuse its existing Baileys record. Repeated
      // connect requests previously created competing sockets for the same phone,
      // which produced connection-replaced errors and frequent disconnects.
      let waba = await WhatsappWaba.findOne(reusableQuery)
        .sort({ updatedAt: -1, createdAt: -1, _id: -1 });
      ...
      if (!waba) {
        ...
        waba.name = finalInstanceName;
        waba.instance_name = finalInstanceName;
        waba.workspace_id = workspace_id || waba.workspace_id || null;
        waba.provider = PROVIDER_TYPES.BAILEY;
        waba.is_active = true;
      ...
      return res.json({
        success: true,
        message: isNewWaba
          ? 'Baileys instance created and initializing. Please fetch QR code.'
          : 'Existing Baileys instance reused and synchronized.',
        data: {
          waba_id: waba._id,
    ...
    const enrichedWabas = await Promise.all(
      wabas.map(async (waba) => {
        const phoneNumbers = await WhatsappPhoneNumber.find({
          ...
          .sort({ is_active: -1, is_primary: -1, updatedAt: -1, createdAt: -1, _id: -1 })
          .lean();
        ...
        const enrichedPhoneNumbers = await Promise.all(
          phoneNumbers.map(async (phone) => {
            ...
            // A Baileys device JID is not a Meta Graph phone-number ID. Calling
            // graph.facebook.com with it caused misleading 401 errors and slowed
            // every database status refresh.
            const isBusinessApi = waba.provider === PROVIDER_TYPES.BUSINESS_API;
            const graphPhoneId = String(phone.phone_number_id || '');
            if (isBusinessApi && /^\d+$/.test(graphPhoneId) && waba.access_token) {
              try {
                const response = await axios.get(
                  `https://graph.facebook.com/v22.0/${graphPhoneId}`,
                  {
                ...
                console.error(
                  `Failed to fetch WhatsApp details for ${graphPhoneId}`,
                  err.message
                );
              }
            }
            ...
            return {
              ...
              last_used_at: phone.last_used_at || null,
              created_at: phone.created_at || phone.createdAt || null,
              updated_at: phone.updated_at || phone.updatedAt || null
            };
          })
        );
        ...
        const runtimeStatus = runtime.status || waba.connection_status || 'unknown';
        ...
        return {
          ...
          connection_status: runtimeStatus,
          stored_connection_status: waba.connection_status || null,
          runtime_connected: runtime.connected === true,
          runtime_state: runtime.runtime_state || null,
          verified_live_socket: runtime.verified_live_socket === true,
          display_phone_number:
            runtime.phone_number ||
            waba.display_phone_number ||
            waba.registred_phone_number ||
            enrichedPhoneNumbers.find((item) => item.is_active)?.display_phone_number ||
            enrichedPhoneNumbers[0]?.display_phone_number ||
            null,
          whatsapp_business_account_id: waba.whatsapp_business_account_id,
          app_id: waba.app_id,
          ...
          created_at: waba.created_at || waba.createdAt || null,
          updated_at: waba.updated_at || waba.updatedAt || null
        };
      })
    );
...
export const getBaileysQRCode = async (req, res) => {
  try {
    ...
    // Status polling never destroys a healthy session. A fresh-session reset is
    // performed only after an explicit force request, or for a known conflict
    // state that cannot produce a usable QR on its own.
    if (
      !qrData?.qr_code &&
      normalizedStatus !== 'connected' &&
      (forceReconnect || recoveryStatuses.has(normalizedStatus))
    ) {
      const providerInstance = unifiedWhatsAppService.providers?.[PROVIDER_TYPES.BAILEY];

      if (typeof providerInstance?.resetConnectionSession === 'function') {
        await providerInstance.resetConnectionSession(wabaId, { deleteSession: true });
      } else {
      ...
      unifiedWhatsAppService
        ...
        .catch((err) =>
          console.error(`Failed to start fresh Baileys QR for WABA ${wabaId}:`, err)
        );
    ...
    const needsInit =
      !qrData?.qr_code &&
      ['disconnected', 'qr_timeout', 'initial', 'generating', 'reconnecting'].includes(
        normalizedStatus
      );
    ...
    if (needsInit) {
      ...
      unifiedWhatsAppService
        ...
        .catch((err) =>
          console.error(`Failed to initialize Baileys QR for WABA ${wabaId}:`, err)
        );
